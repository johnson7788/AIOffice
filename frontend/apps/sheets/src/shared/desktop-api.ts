...
export interface DesktopApi {
  ...
  /**
   * Opens a workbook. `pick` shows the file chooser (File → Open); without it
   * (boot) the web adapter opens the ?open= doc or synthesizes a blank so a new
   * tab never pops a native picker.
   */
  selectWorkbook(pick?: boolean): Promise<WorkbookFile | null>
