...
export function App(): React.JSX.Element {
  ...
  function isAgentConfigured(): boolean {
    ...
    // Web build: the backend resolves the model from its own config, so the
    // renderer's provider settings never gate the agent (mirrors docs).
    if (settings.provider === 'backend') return true
  ...
  useEffect(() => {
    ...
    void window.desktopApi?.hasQueuedWorkbook?.().then(async (queued) => {
      if (queued) await handleInspectWorkbook()
      // Cross-app "生成" handoff from the docs Home: ?gen=<prompt> auto-runs the
      // agent once the (blank) workbook is open. Stripped so a reload won't rerun.
      // Force auto-save: a Home-generated workbook must persist, not live only
      // in the in-memory edit journal (autoSaveCompletedAiRun saves on done).
      ...
      (event) => {
        ...
        if (contentEdited) {
          ...
          if (
            ...
            // sidecar omits sparklines when empty (serde skip_serializing_if)
            state.file.sheets.some((sheet) => (sheet.sparklines?.length ?? 0) > 0)
          ) {
            queueSparklineInstall(
              runtime,
              lazyWorkbookRef,
  ...
  async function handleInspectWorkbook(pick = false): Promise<void> {
    ...
    try {
      ...
      const selected = await window.desktopApi.selectWorkbook(pick)
      if (!selected) {
  ...
  menuActionRef.current = (action) => {
    if (action === 'open') {
      void handleInspectWorkbook(true)
