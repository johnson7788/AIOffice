...
// Web SaaS build of the pdf renderer. Backend (:8585) reached same-origin via the
// dev proxy (prod: nginx serves this under /pdf/ and proxies the API prefixes).
...
export default defineConfig(({ command }) => ({
  ...
  plugins: [react()],
  server: {
    port: Number(process.env.PDF_DEV_PORT) || 3587,
    strictPort: true,
    proxy: {
