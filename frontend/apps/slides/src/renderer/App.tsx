...
export function App() {
  ...
  useEffect(() => {
    ...
    // The docs Home hands off a PPT-generation prompt via ?gen= (cross-origin
    // safe, unlike sessionStorage). On a blank boot, auto-run the AI on it.
    const consumeGenPrompt = () => {
      const gen = new URLSearchParams(location.search).get('gen')
      if (!gen) return
      history.replaceState(null, '', location.pathname)
      setShowAi(true)
      setAiPreset({ text: gen, nonce: Date.now(), autoRun: true })
    }
