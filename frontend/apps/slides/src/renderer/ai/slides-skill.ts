...
const TOOLS: AgentToolDef[] = [
  ...
  },
  {
    name: 'generate_image',
    description:
      'AI image generation/editing (AI Office). Text-to-image, or pass referenceImageUrls for image editing; returns an image URL, then insert with insert_web_image. Use for custom illustrations/icons/backgrounds, style-consistent imagery, and edits like background removal/upscaling/outpainting; for real photos/screenshots still use image_search.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
  ...
  },
  {
    name: 'analyze_media',
    description:
      'Analyze media content (AI Office): understand images/audio/video. Pass media URLs (or local file paths) and analysis requirements; returns analysis text. Video supports extracting key points, structure, and time ranges — good for turning user material into usable deck content.',
    inputSchema: {
      type: 'object',
      properties: {
        mediaUrls: {
...
function blockScratchBuild(
  ...
  if (!cloudAvailable) return null // Web build: cloud deck gen unavailable, native tools are the only path — allow them
  let contentEls = 0
  ...
  switch (call.name) {
    ...
    case 'generate_deck': {
      ...
      if (!access.generatePageCloud || !(await access.isCloudPageGenEnabled?.().catch(() => false)))
        return fail(
          t('aiFailGenDeck'),
          'Cloud slide generation is unavailable in this environment — build the deck with native tools instead (add_text_box/add_shape/set_element_text/edit_element_text), page by page',
        )
      if (!access.generateFromHtml)
        return fail(
          t('aiFailGenDeck'),
          'The current environment does not support the HTML→pptx pipeline — build the deck with native tools instead',
        )
    ...
    case 'add_shape': {
      ...
      const cloudAvailable = await access.isCloudPageGenEnabled?.().catch(() => false)
      const scratchBlock = blockScratchBuild(call.name, slides, state, cloudAvailable)
    ...
    case 'add_smartart': {
      const cloudAvailable = await access.isCloudPageGenEnabled?.().catch(() => false)
      const scratchBlockSA = blockScratchBuild(call.name, slides, state, cloudAvailable)
