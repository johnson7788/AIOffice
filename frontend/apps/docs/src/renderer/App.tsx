  ...
  useEffect(() => {
    ...
    void bootPendingRef.current
      .then(async ([pending]) => {
        ...
        // Home hands off a generation prompt via sessionStorage → autorun the agent
        const prompt = sessionStorage.getItem('aioffice.pendingPrompt')
        if (prompt) {
          sessionStorage.removeItem('aioffice.pendingPrompt')
          setShowAi(true)
