...
from .routers import auth, documents, projects, sharing, sheets
...
app = FastAPI(title="AIOffice backend", version="0.1.0", lifespan=lifespan)
...
app.include_router(sharing.owner_router)
...
async def web_search(query: str, max: int = 6) -> dict:
    return await do_web_search(query, max)


@app.get("/ai/image-search")
async def image_search(query: str, max: int = 8) -> dict:
    return await do_image_search(query, max)
