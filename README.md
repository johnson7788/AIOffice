...
在 `backend/app/skills/<skill-name>/` 下放 `SKILL.md`（分步指导）+ `scripts/`（Python 脚本），然后在 `agent.py` 里 `load_skill_from_dir(...)` 加进 `SkillToolset(skills=[...])`。模型通过 `list_skills`/`load_skill`/`run_skill_script` 自动发现调用。
